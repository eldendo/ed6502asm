...
begin
    if newORG then begin writeln(lnNum,' data ',-2,',',address);inc(lnNum); newORG := false end;
//    writeln(sym,' ',mnem);
...
procedure line;
    ...
    procedure labeldef;
    begin
        if pass=1 then newLabel(text,address);
        getSym;
...
//            if pass=1 then exit;
            tmp := getLabel(text);
            if tmp <> -1 then val := tmp
                         else if pass=1 then val := $FFFF else err('label "'+text+'" not found')
        end;
            ...
            case sym of
                ...
                s_eol: begin 
                            ...
                            if mnem in [mn_BCC,mn_BCS,mn_BEQ,mn_BMI,mn_BNE,mn_BPL,mn_BVC,mn_BVS] 
                                ...
                                else if (val>255) or (mnem in [mn_JMP,mn_JSR]) then mMode := mm_abs else mMode := mm_zp;
// inserted from former emit... check out !!!                       
        (* *)               if (pass<>1) and(mMode = mm_Rel) and fromLabel then
                                    begin
                                        {$R-} 
                                        val := val-address-2; // with rangechecking this gives a runtime error. casting required
                                        {$R+} 
        ...
        procedure pseudo;
            ...
            var i: integer;
            begin
                case sym of // support for a label ?
                    s_num: begin 
                                opc := val; mMode := mm_imp;
                                if pass=2 then emit(address,1,opc,val,newORG);
                                if pass=3 then emit(address,1,opc,val,newORG);
                                inc(address)
                           end;
        ...
        begin
            case mnem of
                pi_ORG: begin consume(s_num); address := val ;newORG := true end;
                pi_EQU: begin consume(s_num); if pass=1 then alterLastLabel(val) end;
                pi_END: sym := s_eof;
                pi_DB: begin 
                            DBitem;
...
//        writeln;writeln(mnem,' ',mMode);
        ...
        if pass=2 then emit(address,nrArg[mMode]+1,opc,val,newORG);
        if pass=3 then emit(address,nrArg[mMode]+1,opc,val,newORG);
...
begin //main
    ...
    getCh; getSym;
//    while sym = s_eol do getSym;
//    if mnem <> pi_ORG then err('ORG expected'); 
