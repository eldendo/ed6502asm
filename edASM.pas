...
// inserted from former emit... check out !!!                       
            ...
            begin
                case sym of // support for a label ?
                    s_num: begin 
                                ...
                                if pass=2 then emit(address,1,opc,val);
                                if pass=3 then emit(address,1,opc,val);
                                inc(address)
                           end;
        ...
        begin
            case mnem of
                pi_ORG: begin consume(s_num); address := val ;newOrg end;
    ...
    begin // instruction
        ...
        if pass=2 then emit(address,nrArg[mMode]+1,opc,val);
        if pass=3 then emit(address,nrArg[mMode]+1,opc,val);
